...
| Decision | レジストリ | スナップショット（`state.json`） |
|----------|------------|----------------------------------|
| **Accept（全行 ACCEPT）** | 変更なし | `CurrentSnapshot` で置換、`CapturedAt` 更新 |
| **Revert（全行 REVERT）** | スナップショットへ書き戻し（余分キー削除含む） | 再 Capture して保存 |
